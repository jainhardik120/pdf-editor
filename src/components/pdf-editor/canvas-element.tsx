...
export const CanvasElement: React.FC<CanvasElementProps> = ({
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (typeof elementId === 'string' && elementId.length > 0 && (element.type === 'row' || element.type === 'column')) {
  ...
  return (
    <div
      className={`group relative transition-all ${isSelected ? 'active-ring' : ''} ${
        cannotMove !== true ? 'cursor-grab active:cursor-grabbing' : 'cursor-not-allowed opacity-60'
      }`}
      draggable={cannotMove !== true}
      onClick={handleClick}
          ...
          {typeof parentId === 'string' && parentId.length > 0 ? (
            <>
              <Button
                ...
                disabled={siblingIndex === 0 || cannotMove === true}
                size="sm"
                variant="ghost"
                onClick={() => {
                  handleMove('up');
              ...
              <Button
                className="h-6 w-6 p-0"
                disabled={siblingIndex === (siblingsCount ?? 0) - 1 || cannotMove === true}
                size="sm"
                variant="ghost"
                onClick={() => {
                  handleMove('down');
